...
class chat
{
    ...
    private function getAllMessageQuery(){
        return "SELECT c.*, sendui.profile as sendPic, sendui.firstname, sendui.lastname, recui.profile as recPic, recui.firstname AS ref, recui.lastname AS rel FROM `chats` AS c INNER JOIN `user` AS sendui INNER JOIN `user` AS recui ON c.sender = sendui.user_id AND c.reciever = recui.user_id WHERE (c.`sender` = ? OR c.`reciever` = ?) AND (c.`sender` = ? OR c.`reciever` = ?) ORDER BY c.created ASC";
    }

    private function getAllWhoMessageQuery(){
        return "SELECT c.*, sendui.profile as sendPic, sendui.firstname, sendui.lastname, recui.profile as recPic, recui.firstname AS ref, recui.lastname AS rel FROM `chats` AS c INNER JOIN `user` AS sendui INNER JOIN `user` AS recui ON c.sender = sendui.user_id AND c.reciever = recui.user_id WHERE c.reciever = ? GROUP BY c.sender ORDER BY created DESC";
    }
}
